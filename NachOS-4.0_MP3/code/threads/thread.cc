...
{
    if (burstTime <= 0)
    return lastExecTime; // means it has terminate (in waiting state), we grab the last execute result.
    else
    return burstTime; // means it just stop (in ready state), we grab the current burst time.
}
...
void Thread::TerminateBurstTimeCounting()
{
    ...
    if (preemtion == 1) {
        predictTime = predictTime - burstTime;
        burstTime = 0.0;
    }
    else {
        double newPredictTime = (double)(predictTime / 2) + (double)(burstTime / 2);
        DEBUG(dbgExpr, "[C] Tick [" << kernel->stats->totalTicks << "]: Thread [" << ID << "] update approximate burst time, from: [" << predictTime << "], add [" << burstTime << "], to [" << newPredictTime << "]");
